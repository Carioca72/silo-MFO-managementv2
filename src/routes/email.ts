...
import { processInbox, extractEmailData } from '../services/ai/emailExtractor.js';
...
router.post('/analyze-inbox', async (_,res) => {
 ...
 const results = await processInbox(mockEmails);
 res.json({results, total:results.length, processedAt:new Date()});
});

// POST /api/email/extract — extrai dados de email especifico
router.post('/extract', async (req,res) => {
 const {event, decision} = req.body;
 const data = await extractEmailData(
 event.from, event.subject||'', event.body, event.attachments||[]
 );
 const io = (global as any).io;
 if (io) io.emit('email:extracted', {from:event.from, data});
 res.json({data, extractedAt:new Date()});
});

let emailSessions: any[] = [
...
router.get('/sessions', (_,res) => res.json(emailSessions));

router.post('/sessions', (req,res) => {
 const {nome,smtpHost,smtpPort,smtpUser,smtpPass,imapHost,imapPort} = req.body;
 ...
 createdAt:new Date().toISOString()};
 emailSessions.push(sess); res.status(201).json(sess);
});

router.get('/sessions/:id/inbox', (_,res) => res.json([
...
router.post('/send', (req,res) => {
 ...
 res.json({success:true,messageId:'email'+Date.now(),ts:new Date()});
});

router.get('/templates', (_,res) => res.json([
