...
export interface TriggerEvent {
 source: 'email'|'whatsapp'|'cron'|'manual';
 from: string; subject?: string; body: string;
 attachments?: string[]; receivedAt: string;
}

export interface TriggerDecision {
 action: 'generate_report'|'extract_data'|'send_material'|'schedule_meeting'|'ignore'|'clarify';
 confidence: number; // 0.0 a 1.0
 intent: string; // intencao detectada
 clientId?: string;
 reportType?: string; // relatorio | estudo | alerta
 tools?: string[]; // endpoints ComDinheiro sugeridos
...
export async function analyzeTrigger(event: TriggerEvent): Promise<TriggerDecision> {
...
SUBJECT: ${event.subject||''}\nBODY: ${event.body.substring(0,500)}`;
 ...
 return JSON.parse(txt) as TriggerDecision;
 } catch {
...
export async function executeTriggerAction(
 decision: TriggerDecision, event: TriggerEvent, io?: any
): Promise<void> {
 if (decision.confidence < 0.6) return;
 const base = `http://localhost:${process.env.PORT||3000}`;
 
 if (io) io.emit('trigger:fired', {
