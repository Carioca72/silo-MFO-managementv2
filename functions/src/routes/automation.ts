...
interface AutoNode { id:string; type:string; label:string; config?:Record<string,any>; }
interface Execution { id:string; status:'running'|'success'|'error'; startedAt:string; finishedAt?:string; log:string[] }
interface Job { id:string; nome:string; cronExpr:string; active:boolean;
 nodes:AutoNode[]; lastRun?:string; nextRun?:string;
 status:'idle'|'running'|'error'; createdAt:string; executions:Execution[]; }
...
router.get('/:id', (req,res) => {
 ...
 if (!job) return res.status(404).json({error:'Nao encontrado'});
 return res.json(job);
});
...
router.post('/', (req,res) => {
 ...
 const job: Job = { id:'j'+Date.now(), nome, cronExpr:cronExpr||'manual',
...
router.post('/:id/toggle', (req,res) => {
 ...
 if (io) io.emit('automation:toggle', {id:job.id, active:job.active});
...
router.post('/:id/run', async (req,res) => {
 const job = jobs.find(j=>j.id===req.params.id);
 if (!job) return res.status(404).json({error:'Nao encontrado'});
 if (job.status==='running') return res.status(409).json({error:'Ja em execucao'});
 
 job.status = 'running';
 const exec: Execution = { id:'ex'+Date.now(), status:'running',
 ...
 if (io) io.emit(`automation:progress:${job.id}`, { nodeId:node.id, label:node.label, log:exec.log });
 await new Promise(r=>setTimeout(r,900));
 }
 job.status = 'idle'; job.lastRun = new Date().toISOString();
 exec.status = 'success'; exec.finishedAt = new Date().toISOString();
 exec.log.push('Concluido com sucesso');
 if (io) io.emit('automation:done', {id:job.id, success:true});
 })();
