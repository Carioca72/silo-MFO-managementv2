...
router.post('/analyze', async (req,res) => {
 ...
 res.json({decision, triggered: decision.confidence >= 0.6});
});
...
router.post('/email-webhook', async (req,res) => {
 ...
 res.json({received:true, action:decision.action, confidence:decision.confidence});
});
...
router.get('/log', (_,res) => res.json(triggerLog.slice(0,50)));

router.get('/stats', (_,res) => {
 const byAction: Record<string,number> = {};
 triggerLog.forEach(l => { byAction[l.decision.action]=(byAction[l.decision.action]||0)+1; });
 res.json({total:triggerLog.length, byAction, lastFired:triggerLog[0]?.ts});
});
